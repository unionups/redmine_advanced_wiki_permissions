  ...
  def self.included(base)
    base.class_eval do
      ...
      def allowed_to?(action, context, options = { })
        ...
        allowed_to_without_wiki_permissions?(action, context, options)
      end
      ...
      def delegate_permission_for(project)
        project.delegate_permissions.find(:first, :conditions => { :user_id => self.id })
      end

      def principals_for_delegate(project)
        return render_403 unless allowed_to_manage_wiki_rights?(project)
        return Principal.active.find(:all, :limit => 100, :order => 'type, login, lastname ASC') if wiki_manager?(project)
